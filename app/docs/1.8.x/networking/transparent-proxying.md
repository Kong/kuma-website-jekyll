...
On **Universal** `kuma-dp` leverages the [data plane proxy specification](../generated/resources/proxy_dataplane) associated to it for receiving incoming requests on a pre-defined port.

There are several advantages for using transparent proxying in universal mode:

 * Simpler [Dataplane resource](../generated/resources/proxy_dataplane), as the `outbound` section becomes obsolete and can be skipped.
