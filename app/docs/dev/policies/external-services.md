...
When you enable this policy, you should also [disable passthrough mode](mesh.md#controlling-the-passthrough-mode) for the mesh and enable the [data plane proxy builtin DNS](../networking/dns) name resolution.
...
* With the real name and port, in this case `curl httpbin.org:443`. This approach works only with [the data plane proxy builtin DNS](../networking/dns) name resolution.
...
In scenarios when traffic to external services needs to be sent through a unique set of hosts you will [configure ZoneEgress](../explore/zoneegress).
