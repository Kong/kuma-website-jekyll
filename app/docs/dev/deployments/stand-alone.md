...
It's possible to run [`ZoneEgress`](../explore/zoneegress) for standalone deployment. In order to deploy Kuma with `ZoneEgress` run the install command with an additional parameter.
```sh
