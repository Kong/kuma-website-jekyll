...
On Kubernetes, Kuma secrets are being stored in the `kuma-system` namespace, while on Universal they are being stored in the underlying [backend](/docs/{{ page.version }}/explore/backends) configured in `kuma-cp`.
